...
const [cPage, sPage] = useState(1); 
  ...
  const changeStatus = async (id) => {
    try {
      const reponse = await axios.patch(`https://cozastore.tryasp.net/api/Products/change-status/${id}`)
      if (reponse.data) {
                  ...
                  {currentItems.map((item, stt) => {
                    ...
                    return (
                      <tr key={stt}>
                        ...
                        <td>
                          <img
                            src={`https://cozastore.tryasp.net/${item.imagePrimary}`}
                            alt={`media-${item.id}`}
                                    ...
                                    <div className="list-images">
                                      ...
                                      {allimages.map((img) => (
                                        <div key={img.id} className="image-item">
                                          <img
                                            src={`https://cozastore.tryasp.net/${img.link}`}
                                            alt="Product"
                                          />
                                          {/* Nút xóa */}
                                          {!img.isImage && (
