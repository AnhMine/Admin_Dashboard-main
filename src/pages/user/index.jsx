...
export default function UserPage() {
  ...
  const fetchCustomers = useCallback(async (pageNumber = 1) => {
    setLoading(true);
    try {
      const response = await axios.get(`https://cozastore.tryasp.net/api/Account/get-all-account?maxPageSize=${maxPageSize}&PageSize=${rowsPerPage}&PageNumber=${page + 1}&Search=${search}`, {
...
const handleApprove = useCallback(async (id) => {
  ...
  try {
    await axios.post(`https://cozastore.tryasp.net/api/Account/un-block-account/${id}`);
...
const handleReject = useCallback(async (id) => {
  ...
  try {
    await axios.post(`https://cozastore.tryasp.net/api/Account/block-account/${id}`);
