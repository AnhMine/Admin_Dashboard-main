...
function EditBannerCollection() {
  ...
  const statusBanner = useSelector(
    (state) => state.bannerCollection.statusGetById
  );
  const dataBanner = useSelector((state) => state.bannerCollection.data);

  useEffect(() => {
    if (id) {
      dispatch(getBannerCollectionById(id));
    }
  }, [id, dispatch]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      dispatch(updateBannerCollection({ bannerId: id, data: values })).then(
        (res) => {
          if (
            res.type === "bannerCollection/updateBannerCollection/fulfilled"
          ) {
            handleToast("success", "Cập nhật banner thành công");
            resetForm();
            navigate("/dashboard/bannercollection");
          } else {
            handleToast("error", "Cập nhật banner thất bại");
          }
        }
      );
    },
